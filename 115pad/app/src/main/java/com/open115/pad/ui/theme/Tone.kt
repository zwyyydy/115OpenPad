...
/*
 * 封面取色：从海报里挑一个"能当整页底色"的色，让每部片/每个库有自己的颜色。
 *
 * ★ **不引 androidx.palette。** 它给的是「Vibrant / Muted」六个槽，而我们只要一个
 *   「能当整页底色」的值 —— 要求比 Palette 更窄：必须够艳（灰了就等于没取色）、
...
/**
 * 从像素里挑一个能当页面底色的色（0xRRGGBB）。全是灰 / 黑白时返回 null。
 *
    ...
    for (p in pixels) {
        ...
        /* 太黑 / 太灰的像素不参与：它们决定不了「这部片是什么颜色」。
           ...
           ☠ **明度不设上限。** 写过 `v > 0.96f` 一版，当场被单测打红：
             ...
             取色恒回落默认色 —— 而界面上只表现为「底色一直是默认的」，不报错。
...
/** 取过的色留一份：翻回上一页、重进详情页时不该再解一次图。 */
private val toneCache = LruCache<String, Color>(64)

/**
 * 这张封面的主色。取不到（没图 / 灰图 / 加载失败）就回落 [fallback]。
 *
 * [model] 是任何 Coil 能吃的 model（本地文件路径、http 直链都行）。
 *
 * ★ 颜色是**渐变过去的**，不是跳过去的：图先到、色后到，跳变会让详情页开场闪一下。
 */
@Composable
fun rememberTone(model: Any?, fallback: Color): Color {
    val ctx = LocalContext.current
    val key = model?.toString()
    var raw by remember(key) { mutableStateOf(key?.let { toneCache[it] }) }

    LaunchedEffect(key) {
        if (key.isNullOrEmpty() || raw != null) return@LaunchedEffect
        val got = withContext(Dispatchers.Default) {
            runCatching {
                val req = ImageRequest.Builder(ctx)
                    .data(model)
                    // ☠ 硬件位图**读不到像素**（getPixels 直接抛）。这一行漏了的话取色恒失败，
                    //   而表现只是「底色一直是默认的」，不报错
                    .allowHardware(false)
                    .size(72, 72)
                    .build()
                // 用 Coil 的**同一份** ImageLoader（应用里已配好 OkHttpClient + 鉴权链），
                // 而不是 new 一个 —— 否则 115 的图片（要带 UA/鉴权）在这里会取不到
                val drawable = (ctx.imageLoader.execute(req) as? SuccessResult)?.drawable
                val bmp = (drawable as? BitmapDrawable)?.bitmap ?: return@runCatching null
        ...
        if (got != null) {
            toneCache.put(key, got)
            raw = got
        }
    }

    val target = raw ?: fallback
    val animated by animateColorAsState(target, tween(420), label = "tone")
    return animated
}
